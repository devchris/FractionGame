...
{
    [AddComponentMenu("Circular Gravity Force/CGF 2D",1)]
    public class CircularGravity2D : MonoBehaviour
    {
        ...
        private IEnumerable<Rigidbody2D> AlignToForceFilter2D(CircularGravityForce.CircularGravity.ConstraintProperties _constraintProperties, IEnumerable<Rigidbody2D> list)
        {
        ...
        private void ApplyAlignment2D(IEnumerable<Rigidbody2D> list)
        {
            foreach (var item in list)
            {
                ...
                switch (_constraintProperties._alignDirection)
                {
                    case CircularGravity.ConstraintProperties.AlignDirection.Up:
                        newLocal = -item.transform.forward;
                        break;
                ...
                Quaternion newRotation = Quaternion.LookRotation((item.transform.position - this.transform.position).normalized, newLocal);
        ...
        public static GameObject CreateCGF()
        {
            ...
            cgf.AddComponent<CircularGravity2D>()._drawGravityProperties.GravityLineMaterial = new Material(Shader.Find("UI/Default"));
